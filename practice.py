...
profile(lang="js", name="hojin", age=23) # 순서가 바껴도 keyword써서 parameter 전달하면 가능.
...
for subject, score in scores.items(): #subject가 key가 되고 score가 value가 됨
    print(subject.ljust(8), str(score).rjust(4), sep=":") #ljust는 왼쪽으로 정렬을 하는데 8칸을 확보한 상태에서.
...
score_file = open("score.txt", "w", encoding="utf8") # w : 쓰기위한 용도 이미 존재한다면 덮어쓰게 된다.
...
print(BattleCruiser.speed) # 563줄에 따라서 AttackFlyUnit안에 speed를 정의하지 않아도 AttackUnit안에 있는 speed variable을 사용 가능하다.
